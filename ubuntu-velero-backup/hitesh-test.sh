...
az storage account create --name $VELERO_STORAGE_ACCOUNT_NAME --resource-group $VELERO_RESOURCE_GROUP_NAME --location $LOCATION --kind StorageV2 --sku Standard_RAGRS --encryption-services blob --https-only true --access-tier Hot
...
schedules:
  veleronamespacessstg:
    disabled: false
    labels:
      myenv: veleronamespacessstg
    annotations:
      myenv: veleronamespacessstg
    schedule: " 30  07 * * * "
...
echo "Staring velero..."
helm install veleronamespacessstg-01 vmware-tanzu/velero --namespace veleronamespacessstg --values=velero-values.yml --version=2.27.2 \
